-- =====================================================================
-- RAW · 05_club_games.sql
-- Tabla: club_games.csv
--
...
FROM TABLE(
  INFER_SCHEMA(
    ...
    FILES => 'club_games.csv'
  )
);

-- 2. Tabla RAW generada desde el propio INFER_SCHEMA (todo STRING)
CREATE OR REPLACE TABLE FOOTBALL.RAW.CLUB_GAMES
  USING TEMPLATE (
    SELECT ARRAY_AGG(
             OBJECT_CONSTRUCT(
...
ALTER TABLE FOOTBALL.RAW.CLUB_GAMES
...
CREATE OR REPLACE PIPE FOOTBALL.RAW.PIPE_CLUB_GAMES
  AUTO_INGEST = TRUE
