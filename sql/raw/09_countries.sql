...
CREATE OR REPLACE TABLE FOOTBALL.RAW.COUNTRIES
  USING TEMPLATE (
    SELECT ARRAY_AGG(
             OBJECT_CONSTRUCT(
...
ALTER TABLE FOOTBALL.RAW.COUNTRIES
...
CREATE OR REPLACE PIPE FOOTBALL.RAW.PIPE_COUNTRIES
  AUTO_INGEST = TRUE
...
ALTER PIPE FOOTBALL.RAW.PIPE_COUNTRIES REFRESH;

-- Validación:
-- SELECT * FROM FOOTBALL.RAW.COUNTRIES;
-- SELECT SYSTEM$PIPE_STATUS('FOOTBALL.RAW.PIPE_COUNTRIES');
