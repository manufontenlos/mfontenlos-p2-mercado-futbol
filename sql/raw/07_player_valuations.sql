-- =====================================================================
-- RAW · 07_player_valuations.sql
-- Tabla: player_valuations.csv
--
...
FROM TABLE(
  INFER_SCHEMA(
    ...
    FILES => 'player_valuations.csv'
  )
);

-- 2. Tabla RAW generada desde el propio INFER_SCHEMA (todo STRING)
CREATE OR REPLACE TABLE FOOTBALL.RAW.PLAYER_VALUATIONS
  USING TEMPLATE (
    SELECT ARRAY_AGG(
             OBJECT_CONSTRUCT(
...
ALTER TABLE FOOTBALL.RAW.PLAYER_VALUATIONS
...
CREATE OR REPLACE PIPE FOOTBALL.RAW.PIPE_PLAYER_VALUATIONS
  AUTO_INGEST = TRUE
